...
{
    $query = "SELECT * FROM laporan_waduk_sempor
                    WHERE
...
{
    $query = "SELECT SUM(tiket_dewasa) as tiket_dewasa, SUM(tiket_anak) as tiket_anak,  SUM(tiket_perahu) as tiket_perahu,  SUM(tiket_r2) as tiket_r2, SUM(tiket_r4) as tiket_r4, SUM(tiket_r6) as tiket_r6, SUM(tiket_retribusi_kios) as tiket_retribusi_kios, SUM(tiket_retribusi_los) as tiket_retribusi_los, SUM(tiket_retribusi_lesehan) as tiket_retribusi_lesehan  FROM laporan_waduk_sempor
                    WHERE
                  tanggal BETWEEN '$keyword' AND '$keyword2'
                ";
...
$ambil = mysqli_query($conn, "SELECT * FROM laporan_waduk_sempor ORDER BY tanggal DESC");

$total = query("SELECT SUM(tiket_dewasa) as tiket_dewasa, SUM(tiket_anak) as tiket_anak,  SUM(tiket_perahu) as tiket_perahu,  SUM(tiket_r2) as tiket_r2, SUM(tiket_r4) as tiket_r4, SUM(tiket_r6) as tiket_r6, SUM(tiket_retribusi_kios) as tiket_retribusi_kios, SUM(tiket_retribusi_los) as tiket_retribusi_los, SUM(tiket_retribusi_lesehan) as tiket_retribusi_lesehan  FROM laporan_waduk_sempor")[0];
...
<head>
    ...
    <title>Detail Laporan Penjualan Tiket Waduk Sempor</title>
...
<body>
    <nav class="navbar">
        ...
        <ul class="navbar-nav">
            <li>
                <h1 class="judulheader" href="#">Detail Laporan Penjualan Tiket Waduk Sempor</h1>
            </li>
    ...
    <div id="main">
        <form action="lapor.php" method="POST">
            <table border="1" width="70%" align="center">
                <tr>
                    <th colspan="17">DAFTAR LAPORAN PENJUALAN TIKET WADUK SEMPOR</td>
                </tr>
                <tr>
                ...
                <?php foreach ($ambil as $row) : ?>
                    <tr>
                        ...
                        <td align="center">
                            <?php $total_3 = $row["tiket_perahu"] * 10000; ?>
                            <?= $row["tiket_perahu"]; ?> = Rp<?= number_format($total_3, 0, ',', '.'); ?>
                        </td>
                        <td align="center">
                        ...
                        <td align="center">
                            <a href="conhapus/hapuswaduksempor.php?id=<?php echo $row["id"] ?>" onclick="return confirm('anda yangkin menghapus data ini?');">Hapus</a>
                        </td>
                    </tr>
                    <?php $i++; ?>
                <?php endforeach; ?>
                <tr>
                    ...
                    <?php $total_3 = $total["tiket_perahu"] * 10000; ?>
                    ...
                    <td align="center">
                        <?= $total["tiket_perahu"]; ?> = Rp<?= number_format($total_3, 0, ',', '.'); ?>
                    </td>
                    <td align="center">
