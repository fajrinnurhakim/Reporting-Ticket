...
{
    $query = "SELECT * FROM laporan_suwuk
                    WHERE
...
{
    $query = "SELECT SUM(tiket_dewasa) as tiket_dewasa, SUM(tiket_anak) as tiket_anak,  SUM(tiket_kuda) as tiket_kuda, SUM(tiket_r2) as tiket_r2, SUM(tiket_r4) as tiket_r4, SUM(tiket_r6) as tiket_r6, SUM(tiket_retribusi_kios) as tiket_retribusi_kios, SUM(tiket_retribusi_los) as tiket_retribusi_los, SUM(tiket_retribusi_lesehan) as tiket_retribusi_lesehan  FROM laporan_suwuk
                    WHERE
                  tanggal BETWEEN '$keyword' AND '$keyword2'
                ";
...
$ambil = mysqli_query($conn, "SELECT * FROM laporan_suwuk ORDER BY tanggal DESC");

$total = query("SELECT SUM(tiket_dewasa) as tiket_dewasa, SUM(tiket_anak) as tiket_anak,  SUM(tiket_kuda) as tiket_kuda, SUM(tiket_r2) as tiket_r2, SUM(tiket_r4) as tiket_r4, SUM(tiket_r6) as tiket_r6, SUM(tiket_retribusi_kios) as tiket_retribusi_kios, SUM(tiket_retribusi_los) as tiket_retribusi_los, SUM(tiket_retribusi_lesehan) as tiket_retribusi_lesehan  FROM laporan_suwuk")[0];
...
<head>
    ...
    <title>Detail Laporan Penjualan Tiket Pantai Suwuk</title>
...
<body>
    <nav class="navbar">
        ...
        <ul class="navbar-nav">
            <li>
                <h1 class="judulheader" href="#">Detail Laporan Penjualan Tiket Pantai Suwuk</h1>
            </li>
    ...
    <div id="main">
        <form action="lapor.php" method="POST">
            <table border="1" width="70%" align="center">
                <tr>
                    <th colspan="17">DAFTAR LAPORAN PENJUALAN TIKET PANTAI SUWUK</td>
                </tr>
                <tr>
                ...
                <?php foreach ($ambil as $row) : ?>
                    <tr>
                        ...
                        <td align="center">
                            <a href="conhapus/hapussuwuk.php?id=<?php echo $row["id"] ?>" onclick="return confirm('anda yangkin menghapus data ini?');">Hapus</a>
                        </td>
                    </tr>
                    <?php $i++; ?>
                <?php endforeach; ?>
